...
int main() {
    ...
    if (!backgroundMusic.openFromFile("assets/sounds/FirstSteps.ogg")) {  // Укажите путь к вашей музыке
        return -1;
    }
    ...
    while (window.isOpen()) {
        ...
        if (currentState == MENU) {
            ...
            if (player.checkCollisionWithStrawberry(level.getStrawberries())) {
                std::cout << "Клубничка собрана!" << std::endl;

                // Уведомление о том, что уровень завершен
                level.loadNewLevel(levelData1, groundTexture, wallTexture, strawberryTexture); // Загружаем новый уровень

                // Сбрасываем позицию игрока на спавн нового уровня
                player.respawn(level.getSpawnPoint());
            }
